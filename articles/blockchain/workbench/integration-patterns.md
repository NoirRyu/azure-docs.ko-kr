...
스마트 컨트랙트는 종종 외부 시스템 및 장치와 통합해야 하는 비즈니스 절차를 나타냅니다.

이러한 절차의 요구 사항 중에는 외부 시스템, 서비스 또는 장치의 데이터가 포함된 분산 원장에서 트랜잭션을 시작해야 할 필요가 있습니다. 또한 외부 시스템이 분산 원장의 스마트 컨트랙트에서 나온 이벤트에 반응해야 합니다.
...
Azure Blockchain Workbench REST API는 분산 원장에서 트랜잭션을 실행하도록 인증된 요청을 보내는 기능을 제공합니다.
...
메시징 통합은 사용자 생성, 컨트랙트 생성 및 컨트랙트에서 트랜잭션 실행과 관련된 트랜잭션의 실행 및 모니터링에 중점을 두고 있으며 주로 *헤드리스* 백 엔드 시스템에서 사용됩니다.
...
이 시나리오에서는 스마트 컨트랙트 내에서 상태 변경이나 특정 트랜잭션 유형 실행과 같은 이벤트가 발생합니다. 이 이벤트는 이벤트 그리드를 통해 하부 소비자에게 브로드캐스팅되며 이러한 소비자는 적절한 작업을 수행합니다.
...
## <a name="one-way-event-delivery-of-a-message-from-an-external-system-to-a-smart-contract"></a>외부 시스템에서 스마트 컨트랙트로의 메시지 단방향 이벤트 전달
...
-   스마트 컨트랙트가 특정 상태로 전환됩니다. 이 상태에서는 외부 시스템이 원하는 작업을 수행할 때까지 함수가 실행되지 않거나 제한된 수만큼 실행될 수 있습니다.
...
-   함수가 실행되고 일반적으로 상태가 수정됩니다. 상태 변경은 스마트 컨트랙트에 반영된 비즈니스 절차를 진행시켜 다른 함수가 적절하게 실행되도록 합니다.
...
일반적인 통합 시나리오는 스마트 컨트랙트의 센서에서 검색된 원격 분석 데이터를 포함합니다. 센서에 의해 전달된 데이터를 기반으로 하여 스마트 컨트랙트은 정보에 입각한 작업을 수행하고 계약 상태를 변경할 수 있습니다.
...
-   해시 및 모든 메타데이터는 *FileAdded*와 같은 스마트 컨트랙트의 함수로 전송됩니다.
...
외부 시스템 또는 장치가 REST 또는 메시지 API를 사용하여 스마트 컨트랙트와 쉽게 상호 작용할 수 있게 하려면 다음을 수행해야 합니다.
...
5. 응용 프로그램 구성 파일과 스마트 컨트랙트이 Azure Blockchain Workbench에 업로드됩니다.
